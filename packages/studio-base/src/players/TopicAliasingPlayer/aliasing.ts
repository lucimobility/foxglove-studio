...
export function aliasPlayerState(
  ...
  attachmentSubscriptions: Im<SubscribeAttachmentPayload[]>,
 ...
 * @returns a new array of subscription payloads with mapped topic names
 */
export const aliasAttachmentSubscriptions = memoizeWeak(
  (
    inputs: Im<AliasingInputs>,
    subcriptions: SubscribeAttachmentPayload[],
  ): SubscribeAttachmentPayload[] => {
    const { aliasMap: mapping } = memos.buildAliases(inputs);
    ...
    return subcriptions.flatMap((sub) => {
      const mappings = inverseMapping.get(sub.name);
      if (mappings) {

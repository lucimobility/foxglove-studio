...
function PanelExtensionAdapter(
  ...
  const partialExtensionContext = useMemo<PartialPanelExtensionContext>(() => {
    ...
    return {
      ...
      writeAttachments: capabilities.includes(PlayerCapabilities.append)
        ? (attachments) => {
          if (!isMounted()) {
            return;
          }
          getMessagePipelineContext().writeAttachments(attachments);
        }
        : undefined,

      writeMetadata: capabilities.includes(PlayerCapabilities.append)
        ? (metadata) => {
          if (!isMounted()) {
            return;
          }
          getMessagePipelineContext().writeMetadata(metadata);
        }
        : undefined,

      terminateWriter: capabilities.includes(PlayerCapabilities.append)
        ? () => {
          if (!isMounted()) {
            return;
          }
          getMessagePipelineContext().terminateWriter();
        }
        : undefined,

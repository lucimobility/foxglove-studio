...
import { Attachment, Immutable } from "@foxglove/studio";
...
// and message to an array of `AttachmentDataItem` objects. The array+objects will be the same by
// reference, as long as topics/datatypes/global variables haven't changed in the meantime.
export function useCachedGetAttachmentDataItems(
  paths: readonly string[],
): (path: string, attachment: Attachment) => AttachmentDataItem[] | undefined {
  ...
  return useCallback(
    (path: string, attachment: Attachment): AttachmentDataItem[] | undefined => {
      ...
      const messagePathDataItems = getAttachmentDataItems(
        attachment,
        filledInPath,
        topicsByName,
        structures,
...
// Exported for tests.
export function getAttachmentDataItems(
...
): AttachmentDataItem[] | undefined {
  const topic = attachmentsByName[filledInPath.topicName];

  // We don't care about messages that don't match the topic we're looking for.
  if (!topic || attachment.name !== filledInPath.topicName) {
    return;
  }
  ...
  for (const item of filledInPath.messagePath) {
    if (item.type === "filter") {
      if (!filterMatches(item, attachment.data)) {
        return [];
      }
    } else {
      break;
  ...
  const queriedData: AttachmentDataItem[] = [];
...
export type AttachmentAndData = {
  attachment: Attachment;
  queriedData: AttachmentDataItem[];
};

export type MessageDataItemsByPath = {
  readonly [key: string]: readonly AttachmentAndData[];
};
...
export function useDecodeMessagePathsForMessagesByTopic(
  ...
  const memoizedPaths = useShallowMemo(paths);
  const cachedGetAttachmentDataItems = useCachedGetAttachmentDataItems(memoizedPaths);
  ...
  return useCallback(
    (messagesByTopic) => {
      const obj: { [path: string]: AttachmentAndData[] } = {};
      for (const path of memoizedPaths) {
        ...
        const messagesForThisPath: AttachmentAndData[] = [];
        obj[path] = messagesForThisPath;

        for (const message of messages) {
          // Add the item (if it exists) to the array.
          const queriedData = cachedGetAttachmentDataItems(path, message);
          if (queriedData) {
    ...
    [memoizedPaths, cachedGetAttachmentDataItems],
  );
}

...
export default function useMetadataByPath(
  paths: string[],
  historySize: number = Infinity,
): MessageDataItemsByPath {
  const memoizedPaths: string[] = useShallowMemo(paths);
  const subscribeMetadata = useMemo(
    () => filterMap(memoizedPaths, (path) => subscribePayloadFromMetadataName(path)),
    [memoizedPaths],
  );

  const metadataByName = PanelAPI.useMetadataByName({
    metadataSubscriptions: subscribeMetadata,
    historySize,
  });
  ...
  return useMemo(
    () => decodeMessagePathsForMessagesByTopic(metadataByName),
    [decodeMessagePathsForMessagesByTopic, metadataByName],
  );
}

...
// and message to an array of `MetadataDataItem` objects. The array+objects will be the same by
// reference, as long as topics/datatypes/global variables haven't changed in the meantime.
export function useCachedGetMetadataDataItems(
  paths: readonly string[],
): (path: string, metadata: Metadata) => MetadataDataItem[] | undefined {
  const { metadataNames: providerTopics, datatypes } = PanelAPI.useDataSourceInfo();
  ...
  const unmemoizedFilledInPaths = useMemo(() => {
    const filledInPaths: Record<string, string> = {};
    for (const path of parsedPaths) {
      filledInPaths[path] = path;
    }
  ...
  log.info(topicsByName);
  log.info(memoizedFilledInPaths);
  log.info(parsedPaths);
  ...
  return useCallback(
    (path: string, metadata: Metadata): MetadataDataItem[] | undefined => {
      ...
      const messagePathDataItems = getMetadataDataItems(
        metadata,
        filledInPath,
        topicsByName,
        structures,
...
// Exported for tests.
export function getMetadataDataItems(
  metadata: Metadata,
  filledInPath: string,
  metadataByName: Record<string, Metadata>,
  structures: Record<string, MessagePathStructureItemMessage>,
): MetadataDataItem[] | undefined {
  const topic = metadataByName[filledInPath];

  // We don't care about messages that don't match the topic we're looking for.
  if (!topic || metadata.name !== filledInPath) {
    return;
  }
  ...
  // for (const item of filledInPath) {
  //   if (item.type === "filter") {
  ...
  const queriedData: MetadataDataItem[] = [];
  ...
  function traverse(
    ...
    const pathItem = filledInPath[pathIndex];
    // const nextPathItem = filledInPath[pathIndex + 1];
    if (!pathItem) {
      // If we're at the end of the `messagePath`, we're done! Just store the point.
      let constantName: string | undefined;
      // const prevPathItem = filledInPath[pathIndex - 1];
      // if (prevPathItem) {
      //   const fieldName = prevPathItem;
      ...
      // const next = structureItem?.nextByName[pathItem];
      traverse(value[pathItem], pathIndex + 1, `${path}.${pathItem}`, undefined);
    }
    // else if (
    //   pathItem.type === "slice" &&
  ...
  if (structure) {
    traverse(metadata.metadata, 0, quoteTopicNameIfNeeded(filledInPath), structure);
  }
  return queriedData;
}

export type MetadataAndData = {
  metadata: Metadata;
  queriedData: MetadataDataItem[];
};

export type MessageDataItemsByPath = {
  readonly [key: string]: readonly MetadataAndData[];
};
...
export function useDecodeMessagePathsForMessagesByTopic(
  ...
  const memoizedPaths = useShallowMemo(paths);
  const cachedGeMetadataDataItems = useCachedGetMetadataDataItems(memoizedPaths);
  ...
  return useCallback(
    (messagesByTopic) => {
      const obj: { [path: string]: MetadataAndData[] } = {};
      for (const path of memoizedPaths) {
        ...
        const messagesForThisPath: MetadataAndData[] = [];
        obj[path] = messagesForThisPath;

        for (const message of messages) {
          // Add the item (if it exists) to the array.
          const queriedData = cachedGeMetadataDataItems(path, message);
          if (queriedData) {
    ...
    [memoizedPaths, cachedGeMetadataDataItems],
  );
}

...
import { Alert, Button, inputBaseClasses, TextField, Tooltip, Typography } from "@mui/material";
...
function WriteAttachment(props: Props) {
  ...
  const [attachmentData, setAttachmentData] = useImmer<string>("");

  const [message, setMessage] = useState<string>("");

  const writeAttachments = useWriteAttachments();

  const { error, parsedObject } = useMemo(() => parseInput(attachmentData), [attachmentData]);
  ...
  const onAddAttachmentClicked = useCallbackWithToast(() => {
    log.info(parsedObject);

    const data = new TextEncoder().encode(parsedObject);
    const attachment: Attachment = { name: attachmentName, mediaType: config.datatype!, data };

    log.info("writing attachments from attachment panel: ", attachment);

    writeAttachments([attachment]);

    setMessage("Attachment was created, don't forget to save and close the file before closing Foxglove in order to save your changes.");

    // clear entries after written
    setAttachmentData("");
    setAttachmentName("");
  }, [attachmentName, config.datatype, parsedObject, setAttachmentData, writeAttachments]);
        ...
        <Stack flexGrow="1">
          ...
          <Stack flexGrow="1">
            <TextField
              ...
              onChange={(event) => {
                setAttachmentData(event.target.value);
              }}

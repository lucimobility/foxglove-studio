...
import { diffLabels } from "@foxglove/studio-base/panels/ViewAttachments/getDiff";
...
function ViewAttachments(props: Props) {
  ...
  const { attachmentNames, datatypes } = useDataSourceInfo();
  ...
  const rootStructureItem: MessagePathStructureItem | undefined = useMemo(() => {
    if (!topic || !attachmentName) {
      return;
    }
    return traverseStructure(structures[topic.schemaName]).structureItem;
  }, [attachmentName, structures, topic]);
  ...
  log.info("matched attachments: ", matchedMessages);
  ...
  const renderSingleTopicOrDiffOutput = useCallback(() => {
    ...
    return (
      ...
      >
        <Metadata
          data={data}
          attachment={baseItem.attachment}
        />
  ...
  }, [attachmentName, baseItem, classes.topic, fontSize, getItemString, jsonTreeTheme, expansion, onLabelClick, valueRenderer, rootStructureItem]);
